...
calculate_iv_v_returns <- function(iv){
  ...
  iv_rt <- iv[,.(sum_mv = sum(MV)),by=.(iv_rank,yearmon)][order(+yearmon)]
  iv_rt <- iv_rt[,portfolio_return:= log(sum_mv)-shift(log(sum_mv),1L, type="lag"),
                 by=.(iv_rank)]
  iv_rt <- na.omit(iv_rt[,-("sum_mv"),with=F])
